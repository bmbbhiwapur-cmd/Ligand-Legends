...
st.markdown("<p style='text-align: center; font-size:12px; color:gray;'>Powered by InSilico BioSphere | Developed by Sarang Dhote & Students, Shri Shivaji Science College, Nagpur, India.</p>", unsafe_allow_html=True)
...
=========================================
Developed by Sarang Dhote & Students, 
Shri Shivaji Science College, Nagpur, India.

[ RESULT SUMMARY ]
💊 Drug / Ligand  : {drug_n}
🎯 Protein Target : {prot_n}
🩺 Disease Cat.   : {disease_n}
🔥 Affinity Score : {st.session_state.affinity_score} kcal/mol ({rank})
🎲 Selected Pose  : Pose {selected_mode}

[ MACROMOLECULE DETAILS ]
Organism          : {st.session_state.protein_meta.get('organism', 'N/A')}
Method            : {st.session_state.protein_meta.get('method', 'N/A')}
Resolution        : {st.session_state.protein_meta.get('resolution', 'N/A')}

[ LIGAND DETAILS ]
IUPAC Name        : {st.session_state.ligand_meta.get('iupac', 'N/A')}
Formula           : {st.session_state.ligand_meta.get('formula', 'N/A')}
SMILES Code       : {st.session_state.scanned_smiles}

[ GRID CONFIGURATION ]
Center (X, Y, Z)  : {st.session_state.grid_config.get('cx', 0):.2f}, {st.session_state.grid_config.get('cy', 0):.2f}, {st.session_state.grid_config.get('cz', 0):.2f}
Size   (X, Y, Z)  : {st.session_state.grid_config.get('sx', 0):.2f}, {st.session_state.grid_config.get('sy', 0):.2f}, {st.session_state.grid_config.get('sz', 0):.2f}

[ INTERACTION PROFILE (3.8 Å) ]
{interaction_str}
========================================="""

    import urllib.parse
    encoded_msg = urllib.parse.quote(detailed_report)
    ...
    with col1:
        ...
        <a href="{whatsapp_url}" target="_blank" style="text-decoration: none;">
            <div style="background-color: #25D366; color: white; padding: 10px; border-radius: 8px; text-align: center; font-weight: bold; margin-bottom: 10px;">
                💬 Share to WhatsApp
            </div>
        </a>
        """, unsafe_allow_html=True)
    with col2:
        st.download_button(
            label="💾 Download Study Note",
            data=detailed_report,
            file_name=f"{drug_n.replace(' ', '_')}_Docking_Report.txt",
            mime="text/plain",
            use_container_width=True
        )
        
    st.info("📸 **Tip for Mobile Users:** Simply take a SCREENSHOT of your phone to save the 3D complex and your score!")
